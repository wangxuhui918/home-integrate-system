...
public class IdUtils {
    ...
    // 机器ID(0-1023)
    private static long workerId = BaseEv.SettingInformation.distributedKey;
    // 各种元数据
...
//            log.error(String.format("clock is moving backwards.  Rejecting requests until %d.", lastTimestamp));
     ...
     * @return 封装的ID类
     */
    public static ID expId(long id) {
        return IdConverterUtils.convert(id);
    }
    ...
    public static long makeId(long timeStamp, long worker, long sequence) {
        return IdConverterUtils.convert(new ID(timeStamp, worker, sequence));
    }
}
