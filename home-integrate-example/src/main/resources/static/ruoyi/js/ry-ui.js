    ...
    $.extend({
        ...
        table: {
            ...
            responseHandler: function (res) {
                ...
                if (res.codeBody.type == web_status.SUCCESS) {
                    ...
                    $.modal.alertWarning(res.codeBody.text);
                    return {rows: [], total: 0};
                }
            },
            // 初始化事件
            ...
            exportExcel: function (formId) {
                ...
                $.modal.confirm("确定导出所有" + table.options.modalName + "吗？", function () {
                    ...
                    $.post(table.options.exportUrl, dataParam, function (result) {
                        if (result.codeBody.type == web_status.SUCCESS) {
                            window.location.href = ctx + "common/download?fileName=" + encodeURI(result.codeBody.text) + "&delete=" + true;
                        } else if (result.codeBody.type == web_status.WARNING) {
                            $.modal.alertWarning(result.codeBody.text)
                        } else {
                            $.modal.alertError(result.codeBody.text);
                        }
                        $.modal.closeLoading();
                    });
                });
                        ...
                        $.ajax({
                            ...
                            success: function (result) {
                                if (result.codeBody.type == web_status.SUCCESS) {
                                    $.modal.close(index);
                                    $.modal.closeAll();
                                    $.modal.alertSuccess(result.codeBody.text);
                                    $.table.refresh();
                                } else if (result.codeBody.type == web_status.WARNING) {
                                    $.modal.close(index);
                                    $.modal.enable();
        ...
        operate: {
            ...
            submitsync: function (url, type, dataType, data, callback) {
                var config = {
                    url: url,
                    type: type,
                    dataType: dataType,
            ...
            postsync: function (url, data, callback) {
                $.operate.submitsync(url, "post", "json", data, callback);
            },
            // get请求传输
            ...
            editUrl: function (id) {
                ...
                if ($.common.isNotEmpty(id)) {
                    // url = table.options.updateUrl.replace("{id}", id);
                    url = table.options.updateUrl.replace("{id}", "?id=" + id);
                } else {
                    var id = $.common.isEmpty(table.options.uniqueId) ? $.table.selectFirstColumns() : $.table.selectColumns(table.options.uniqueId);
                    if (id.length == 0) {
                        $.modal.alertWarning("请至少选择一条记录");
            ...
            ajaxSuccess: function (result) {
                if (result.codeBody.type == web_status.SUCCESS && table.options.type == table_type.bootstrapTable) {
                    $.modal.msgSuccess(result.codeBody.text);
                    $.table.refresh();
                } else if (result.codeBody.type == web_status.SUCCESS && table.options.type == table_type.bootstrapTreeTable) {
                    $.modal.msgSuccess(result.codeBody.text);
                    $.treeTable.refresh();
                } else if (result.codeBody.type == web_status.SUCCESS && $.common.isEmpty(table.options.type)) {
                    $.modal.msgSuccess(result.codeBody.text)
                } else if (result.codeBody.type == web_status.WARNING) {
                    $.modal.alertWarning(result.codeBody.text)
                } else {
            ...
            saveReload: function (result) {
                if (result.codeBody.type == web_status.SUCCESS) {
                    $.modal.msgSuccessReload(result.codeBody.text);
                } else if (result.codeBody.type == web_status.WARNING) {
                    $.modal.alertWarning(result.codeBody.text)
                } else {
                    $.modal.alertError(result.codeBody.text);
            ...
            successCallback: function (result) {
                if (result.codeBody.type == web_status.SUCCESS) {
                    ...
                    if ($.common.isEmpty(parent.table)) {
                        ...
                        $.modal.close();
                        parent.$.modal.msgSuccess(result.codeBody.text);
            ...
            successTabCallback: function (result) {
                if (result.codeBody.type == web_status.SUCCESS) {
                    ...
                    $.modal.close();
                    $contentWindow.$.modal.msgSuccess(result.codeBody.text);
