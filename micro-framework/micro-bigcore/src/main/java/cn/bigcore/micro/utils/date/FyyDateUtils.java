...
public final class FyyDateUtils {
    ...
    public static String getDateTimePattern() {
        return FyyDateUtils.getDatePattern() + " HH:mm:ss.S";
    }
    ...
    public static Date getMaxTimeOfDate(Date theDate) {
        String sDate = FyyDateUtils.convertDateToString(theDate);
    ...
    public static Date getCalculatedWorkDay(Date baseDate, int offset) {
        ...
        if (offset > 0) {
            int steps = 1;
            do {
                target = FyyDateUtils.addDays(target, 1);
                if (!FyyDateUtils.isWeekend(target))
                    steps++;
