...
public class FyyLineDefaultManager implements FyyLineManagerInterface {
    ...
    public void before() {
        ...
        if (FyyInitEnv.isClassModel) {//仅在class模式下需要处理的东西
    ...
    public void start() {
        //读取配置参数
        {
            ...
            for (int i = 0; i < runmodelconfigrerules.size(); i++) {
                ...
                if (info instanceof FyyLineSystemInterface) {
                    FyyInitEnv.setting.putAll(info.getProperties());
                }
            }
            //读取第三方参数
            for (int i = 0; i < runmodelconfigrerules.size(); i++) {
        ...
        for (int i = 0; i < runmodelconfigrerules.size(); i++) {
            ...
            if (info1 instanceof FyyLineThirdExtendInterface) {
                for (FyyLineRuntimeModelInterface info : runmodelconfigrerules) {
                    if (info instanceof FyyLineThirdExtendInterface) {
                        ((FyyLineThirdExtendInterface) info).callSetting(FyyInitEnv.setting);
                    }
                }
            }
        }
        {
            ...
            {
                Map<String, String> notBlankMap = MapUtil.filter(new HashMap<>(FyyInitEnv.setting), t -> StrUtil.isNotBlank(t.getValue()));
                FyyLogBaseUtils.debug("清空并重新压入项目变量");
                FyyInitEnv.setting.clear();
                FyyInitEnv.setting.putAll(notBlankMap);
            }
            //写入系统变量
            for (String key : FyyInitEnv.setting.keySet()) {//防止一些额外的变量未写入到系统参数中
                FyyLogBaseUtils.debug("压入系统变量[{}]:[{}]", key, FyyInitEnv.setting.get(key));
                System.setProperty(key, FyyInitEnv.setting.get(key));
            }
        }
    ...
    public void after() {
        ...
        for (int i = 0; i < runmodelconfigrerules.size(); i++) {
            ...
            if (info instanceof FyyLineSystemInterface) {
                FyyInitEnv.iocclasses.addAll(info.writeClasss());
            }
        }
        for (int i = 0; i < runmodelconfigrerules.size(); i++) {
            FyyLineRuntimeModelInterface info = runmodelconfigrerules.get(i);
        ...
        if (FyyInitEnv.isClassModel) {//提前处理非Jar模式需要处理的东西
            for (int i = 0; i < classmodelconfigrerules.size(); i++) {
                FyyLineSourceModelInterface info = classmodelconfigrerules.get(i);
                FyyLogBaseUtils.debug("功能处理器[{}]开始处理[{}]", info.getClass(), "writeClasss");
                FyyInitEnv.iocclasses.addAll(info.writeClasss());
