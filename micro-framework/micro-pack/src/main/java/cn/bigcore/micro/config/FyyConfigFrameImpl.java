...
public class FyyConfigFrameImpl implements FyyConfigFrameInterface {
    ...
    public <T> List<String> getPlugins(Class<T> a) {//获取功能插件列表
        ...
        plugins.addAll(FyyAnnotationTools.getRuleOn(FyyRuleInjection.class, a, FyyInitEnv.projectPackage));//加載扩展插件属性信息
    ...
    public List<FyyConfigEntryVo> getPropertiesMain() {//获取主配置文件
        ...
        }
        {
            ...
            plugins.addAll(FyyAnnotationTools.getRuleOn(FyyRuleInjection.class, FyyConfigPropertiesInterface.class, FyyInitEnv.projectPackage));//加載扩展插件属性信息
            for (int i = 0; i < plugins.size(); i++) {
