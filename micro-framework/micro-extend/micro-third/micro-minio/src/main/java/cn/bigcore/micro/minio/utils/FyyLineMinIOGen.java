...
public class FyyLineMinIOGen {
    ...
    public static void intiConect() {
        if (client == null) {
            namespace_re = FyyInitEnv.ProjectInformation.setting.get(FyyConfigEntryDetailsValues.HOME_MINIO_NAMESPACE_RE.getKey());
            client = MinioClient.builder().endpoint(FyyInitEnv.ProjectInformation.setting.get(FyyConfigEntryDetailsValues.HOME_MINIO_ENDPOINT.getKey())).credentials(FyyInitEnv.ProjectInformation.setting.get(FyyConfigEntryDetailsValues.HOME_MINIO_ACCESSKEY.getKey()), FyyInitEnv.ProjectInformation.setting.get(FyyConfigEntryDetailsValues.HOME_MINIO_SECRETKEY.getKey())).build();
        }
    }

    public static void checkMinIOState() {
        if (client == null) {
            throw new FyyExceptionError("MinIO连接失败,请检查相关参数是否正常,{},{},{},{}", FyyInitEnv.ProjectInformation.setting.get(FyyConfigEntryDetailsValues.HOME_MINIO_NAMESPACE_RE.getKey()), FyyInitEnv.ProjectInformation.setting.get(FyyConfigEntryDetailsValues.HOME_MINIO_ENDPOINT.getKey()), FyyInitEnv.ProjectInformation.setting.get(FyyConfigEntryDetailsValues.HOME_MINIO_ACCESSKEY.getKey()), FyyInitEnv.ProjectInformation.setting.get(FyyConfigEntryDetailsValues.HOME_MINIO_SECRETKEY.getKey()));
        }
    }
    ...
    public static String createNameSpace726() {
        ...
        try {
            ...
            if (!isExist) {
                ...
                if (FyyInitEnv.ProjectInformation.setting.getBool(FyyConfigEntryDetailsValues.HOME_MINIO_NAMESPACE_ACCESS_OPEN.getKey())) {
