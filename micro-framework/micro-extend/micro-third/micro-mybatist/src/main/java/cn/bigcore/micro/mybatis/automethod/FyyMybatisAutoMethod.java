...
public class FyyMybatisAutoMethod {
    ...
    public Object annotationAround(ProceedingJoinPoint jp) throws Throwable {
        ...
        if (myAnnotation != null && input != null && FyyInitEnv.ProjectInformation.methTree.get(methFullPath) == null) {
            ...
            FyyInitEnv.ProjectInformation.methTree.put(methFullPath, className);
        }


        if (myAnnotation != null && input != null && FyyInitEnv.ProjectInformation.methTree.get(methFullPath) != null) {
            Mapper mapper = (Mapper) FyyBean.getBean(FyyInitEnv.ProjectInformation.methTree.get(methFullPath).getId());
            if (myAnnotation.method() == FyyMybatisMethod.pageSelect) {
                Object inputvo = input.getData();
                Object domain = ClassUtil.loadClass(FyyInitEnv.ProjectInformation.methTree.get(methFullPath).getName().toString(), false).newInstance();
