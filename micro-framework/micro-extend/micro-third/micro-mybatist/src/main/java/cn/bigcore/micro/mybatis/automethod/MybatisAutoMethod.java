...
import cn.bigcore.micro.mybatis.annotation.RuleInjectionMehtod;
...
public class MybatisAutoMethod {
    ...
    public Object annotationAround(ProceedingJoinPoint jp) throws Throwable {
        ...
        if (myAnnotation != null && input != null && BaseEv.SettingInformation.methTree.get(methFullPath) == null) {
            ...
            if (myAnnotation.method() == cn.bigcore.micro.mybatis.annotation.bean.Method.nomethod) {
        ...
        if (myAnnotation != null && input != null && BaseEv.SettingInformation.methTree.get(methFullPath) != null) {
            ...
            if (myAnnotation.method() == cn.bigcore.micro.mybatis.annotation.bean.Method.pageSelect) {
...
//                Object vo = input.getData().getClass().newInstance();
                ...
                return pageData;
            } else if (myAnnotation.method() == cn.bigcore.micro.mybatis.annotation.bean.Method.andSelect) {
                Object domain = ClassUtil.loadClass(BaseEv.SettingInformation.methTree.get(methFullPath).getName().toString(), false).newInstance();
                List returnlist = new ArrayList<>();
                {
                    returnlist = BeanUtil.copyToList(mapper.select(domain), input.getData().getClass());
                ...
                return returnlist;
            } else if (myAnnotation.method() == cn.bigcore.micro.mybatis.annotation.bean.Method.add) {
                Object domain = ClassUtil.loadClass(BaseEv.SettingInformation.methTree.get(methFullPath).getName().toString(), false).newInstance();
                Object inputvo = input.getData();
                BeanUtil.copyProperties(inputvo, domain);
                return mapper.insert(domain);
            } else if (myAnnotation.method() == cn.bigcore.micro.mybatis.annotation.bean.Method.queryByPrimaryKey) {
                Object domain = ClassUtil.loadClass(BaseEv.SettingInformation.methTree.get(methFullPath).getName().toString(), false).newInstance();
                Object inputvo = input.getData();
                BeanUtil.copyProperties(inputvo, domain);
                return mapper.selectByPrimaryKey(domain);
            } else if (myAnnotation.method() == cn.bigcore.micro.mybatis.annotation.bean.Method.deleteByPrimaryKey) {
                Object domain = ClassUtil.loadClass(BaseEv.SettingInformation.methTree.get(methFullPath).getName().toString(), false).newInstance();
                Object inputvo = input.getData();
                BeanUtil.copyProperties(inputvo, domain);
                return mapper.deleteByPrimaryKey(domain);
            } else if (myAnnotation.method() == cn.bigcore.micro.mybatis.annotation.bean.Method.updateByPrimaryKeySelective) {
                Object domain = ClassUtil.loadClass(BaseEv.SettingInformation.methTree.get(methFullPath).getName().toString(), false).newInstance();
                Object inputvo = input.getData();
                BeanUtil.copyProperties(inputvo, domain);
