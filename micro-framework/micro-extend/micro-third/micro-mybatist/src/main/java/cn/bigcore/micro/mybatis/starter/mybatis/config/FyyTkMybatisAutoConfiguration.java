...
//    @ConditionalOnMissingBean(SqlSessionFactory.class) // 容器中如果没有这个类,那么自动配置这个类
    public SqlSessionFactory sqlSessionFactoryBean(@Qualifier("druidDataSource") DataSource dataSource) {
        ...
        bean.setTypeAliasesPackage(FyyInitEnv.SettingInformation.setting.get(FyyConfigEntryDetailsValues.HOME_MYBATIS_TYPEALIASESPACKAGE.getKey()));
        // 分页插件
        ...
        try {
            Resource[] s = resolver.getResources(FyyInitEnv.SettingInformation.setting.get(FyyConfigEntryDetailsValues.HOME_MYBATIS_MAPPERLOCATIONS.getKey()));
        } catch (Exception e) {
            e.printStackTrace();
            throw new FyyExceptionError(
                    "请在资源目录下创建文件夹并创建Mapper.xml文件:" + FyyInitEnv.SettingInformation.setting.get(FyyConfigEntryDetailsValues.HOME_MYBATIS_MAPPERLOCATIONS.getKey()));
        }
        try {
            bean.setMapperLocations(
                    resolver.getResources(FyyInitEnv.SettingInformation.setting.get(FyyConfigEntryDetailsValues.HOME_MYBATIS_MAPPERLOCATIONS.getKey())));
