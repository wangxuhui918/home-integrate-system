...
public class FyyMybatisPlusAutoConfiguration {
    ...
    public SqlSessionFactory sqlSessionFactoryBean(@Qualifier("druidDataSource") DataSource dataSource) {
        ...
        bean.setTypeAliasesPackage(FyyInitEnv.setting.get(FyyConfigEntryDetailsValues.HOME_MYBATISPLUS_TYPEALIASESPACKAGE.getKey()));
        // 分页插件
        ...
        try {
            Resource[] s = resolver.getResources(FyyInitEnv.setting.get(FyyConfigEntryDetailsValues.HOME_MYBATISPLUS_MAPPERLOCATIONS.getKey()));
        } catch (Exception e) {
            e.printStackTrace();
            throw new FyyExceptionError(
                    "请在资源目录下创建文件夹并创建Mapper.xml文件:" + FyyInitEnv.setting.get(FyyConfigEntryDetailsValues.HOME_MYBATISPLUS_MAPPERLOCATIONS.getKey()));
        }
        try {
            bean.setMapperLocations(
                    resolver.getResources(FyyInitEnv.setting.get(FyyConfigEntryDetailsValues.HOME_MYBATISPLUS_MAPPERLOCATIONS.getKey())));
