...
package com.guoshiyao.rely.config.core.project;
...
import com.guoshiyao.rely.core.utils.properties.PropertiesUtils;
 ...
 * @readme
 */
public class ProjectBuiltInPropertiesImpl implements IProjectConf {
    ...
    public HashMap<String, String> reloadPropertiesValue() {
        ...
        try {
            List<URI> listUrl = ResourceFindUtils.findUri("application-*.properties");//Line.env.getName()
            ...
            }
            {
                ...
                for (int j = 0; j < groups.size(); j++) {
                    ...
                    if (s.equals(BaseEv.SettingInformation.runEnv)) {
...
//                        lineStr.add(StrUtil.format("[{}-{}]", mainConfig.getName(), envName));//格式为 code-env
                ...
                {//写入文件
                    String file = BaseEv.WorkDir.projectresourcepath + FileUtil.FILE_SEPARATOR + "application-" + envName + ".properties";
                    if (!FileUtil.exist(file)) {
