...
//    @ConditionalOnMissingBean(SqlSessionFactory.class) // 容器中如果没有这个类,那么自动配置这个类
    public SqlSessionFactory sqlSessionFactoryBean(@Qualifier("druidDataSource") DataSource dataSource) {
        ...
        bean.setTypeAliasesPackage(BaseEv.SettingInformation.setting.get(ConfigDetails.HOME_MYBATIS_TYPEALIASESPACKAGE.getKey()));
        // 分页插件
        ...
        try {
            Resource[] s = resolver.getResources(BaseEv.SettingInformation.setting.get(ConfigDetails.HOME_MYBATIS_MAPPERLOCATIONS.getKey()));
        } catch (Exception e) {
            e.printStackTrace();
            throw new ExceptionError(
                    "请在资源目录下创建文件夹并创建Mapper.xml文件:" + BaseEv.SettingInformation.setting.get(ConfigDetails.HOME_MYBATIS_MAPPERLOCATIONS.getKey()));
        }
        try {
            bean.setMapperLocations(
                    resolver.getResources(BaseEv.SettingInformation.setting.get(ConfigDetails.HOME_MYBATIS_MAPPERLOCATIONS.getKey())));
