...
public class GitUtilsV2 {
     ...
     */
    @Deprecated
    public GitUtilsV2(String gitDir, String username, String password, String codeHttpUrl) {
        this(gitDir, username, password, codeHttpUrl, null, null, null, null, null);
    }

    public GitUtilsV2(String gitDir, String username, String password, String codeHttpUrl, String token, String name,
                      String desc, String[] groups, String gitlabUrl) {
        this.gitDir = gitDir;
    ...
    public boolean delete15Tag() {
        try {
            ...
            List<GitLabTag> listTags = new ArrayList<>();
            int s = 0;
            ...
            List<GitLabTag> sortTags = listTags.stream().sorted(Comparator.comparing(GitLabTag::getTime))
                    .collect(Collectors.toList());
            Collections.reverse(sortTags);
            List<GitLabTag> dontdelete = new ArrayList<>();
            List<GitLabTag> masterTags = new ArrayList<>();
            List<GitLabTag> mastertestTags = new ArrayList<>();
            List<GitLabTag> releaseTags = new ArrayList<>();
            List<GitLabTag> developTags = new ArrayList<>();
            {// master
                // 90天内的tag
                List<GitLabTag> master = sortTags.stream().filter(st -> {
                    return st.getTime() >= time.getTime() && st.getTagname().startsWith("refs/tags/master/");
                }).collect(Collectors.toList());
                // 90天外的tag
                List<GitLabTag> tags90waiday = sortTags.stream().filter(st -> {
                    return st.getTime() < time.getTime() && st.getTagname().startsWith("refs/tags/master/");
                }).collect(Collectors.toList());
                // 如果90天内少于10个,则补全10个
                if (master.size() < 10) {
            ...
            {// mastertest
                List<GitLabTag> mastertest = sortTags.stream().filter(st -> {
            ...
            {// release
                List<GitLabTag> release = sortTags.stream().filter(st -> {
            ...
            {// develop
                List<GitLabTag> develop = sortTags.stream().filter(st -> {
            ...
            List<GitLabTag> deleteTag = sortTags.stream().filter(item -> !dontdelete.stream().map(e -> e.getId())
            ...
            for (GitLabTag delete : deleteTag) {
    ...
    public boolean commit(String branch) {
        try {
            ...
            if (this.checkOut(branch)) {
                ...
                gitv2.commit().setMessage("HomeStarter标签:" + new SimpleDateFormat("yyMMdd.HHmm").format(new Date()))
